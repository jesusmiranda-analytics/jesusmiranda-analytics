<div align="center">

![Header](https://capsule-render.vercel.app/api?type=waving&color=0:312e81,45:6d28d9,100:a855f7&height=230&section=header&text=Jesús%20Alfonso%20Miranda%20Ortega&fontSize=38&fontColor=ffffff&fontAlignY=36&desc=Data%20Analyst%20%7C%20Business%20Intelligence%20%7C%20Applied%20AI%20and%20ML&descSize=18&descAlignY=56&animation=fadeIn)

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Inter&weight=600&size=22&duration=3000&pause=900&color=A78BFA&center=true&vCenter=true&width=850&lines=Turning+complex+data+into+business+decisions;Building+dashboards%2C+models%2C+and+actionable+insights;Connecting+analytics%2C+AI%2C+and+commercial+strategy)](https://git.io/typing-svg)

[![Tecnológico de Monterrey](https://img.shields.io/badge/Tecnológico_de_Monterrey-Marketing_%2B_Business_Intelligence-4F46E5?style=flat-square&logo=academia&logoColor=white)](https://tec.mx/)
[![University of Chicago](https://img.shields.io/badge/University_of_Chicago-ML_for_Finance-6D28D9?style=flat-square&logo=academia&logoColor=white)](https://www.uchicago.edu/)
[![McKinsey Forward](https://img.shields.io/badge/McKinsey.org-Forward_Program-7C3AED?style=flat-square&logo=academia&logoColor=white)](https://www.mckinsey.com/forward)
[![Location](https://img.shields.io/badge/Querétaro-México-4338CA?style=flat-square&logo=googlemaps&logoColor=white)](https://www.google.com/maps/place/Quer%C3%A9taro,+Mexico)

[![Portfolio](https://img.shields.io/badge/PORTFOLIO-Visit-7C3AED?style=for-the-badge&logo=vercel&logoColor=white)](https://mi-portafolio-xi-ten.vercel.app)
[![LinkedIn](https://img.shields.io/badge/LINKEDIN-Connect-4F46E5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/jes%C3%BAs-alfonso-miranda-ortega-98a381321/)
[![Email](https://img.shields.io/badge/EMAIL-Contact-6D28D9?style=for-the-badge&logo=gmail&logoColor=white)](mailto:jesusm.12@yahoo.com)
[![GitHub](https://img.shields.io/badge/GITHUB-Follow-312E81?style=for-the-badge&logo=github&logoColor=white)](https://github.com/jesusmiranda-analytics)

![Profile Views](https://komarev.com/ghpvc/?username=jesusmiranda-analytics&label=PROFILE+VIEWS&color=7c3aed&style=flat-square)
[![Followers](https://img.shields.io/github/followers/jesusmiranda-analytics?label=FOLLOWERS&style=flat-square&color=6366F1)](https://github.com/jesusmiranda-analytics?tab=followers)
[![Stars](https://img.shields.io/github/stars/jesusmiranda-analytics?affiliations=OWNER&label=TOTAL+STARS&style=flat-square&color=A855F7)](https://github.com/jesusmiranda-analytics?tab=repositories)

</div>

---

## About

I am a **Data Analyst and Business Intelligence professional in training** focused on turning complex data into clear, actionable business decisions. My work spans data preparation, KPI design, executive dashboards, statistical analysis, predictive modeling, machine learning, CRM analytics, and process automation.

As a dual-degree student in **Marketing and Business Intelligence at Tecnológico de Monterrey**, I combine quantitative analysis with commercial and strategic context. My approach is to understand the decision first, build a trustworthy analytical process, validate the evidence, and communicate insights in a way that stakeholders can act on.

- **Data Analysis & BI:** Python, SQL, R, MySQL, PostgreSQL, Power BI, Tableau, Looker, SPSS, KPI frameworks, and executive reporting.
- **AI / ML:** supervised learning, ensemble models, probability calibration, clustering, forecasting, anomaly detection, and human-in-the-loop analysis.
- **Business Analytics:** CRM and customer analytics, supply-chain KPIs, service-quality diagnostics, segmentation, CLV, and strategic recommendations.
- **Technical Delivery:** reproducible data pipelines, ETL automation, data validation, APIs, dashboards, and documented end-to-end analytical workflows.

### Open To

Data Analyst, Business Intelligence Analyst, Junior Data Scientist, AI/ML Analyst, analytics consulting, and high-impact projects connecting data with business strategy.

---

## Tech Stack

<div align="center">

### Data & Querying

[![Data and Querying](https://skillicons.dev/icons?i=python,r,mysql,postgres&theme=dark)](https://skillicons.dev)

### Business Intelligence & Analytics

![Power BI](https://img.shields.io/badge/Power_BI-312E81?style=flat-square&logo=powerbi&logoColor=white)
![Tableau](https://img.shields.io/badge/Tableau-4338CA?style=flat-square&logo=tableau&logoColor=white)
![Looker](https://img.shields.io/badge/Looker-4F46E5?style=flat-square&logo=looker&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-6366F1?style=flat-square&logo=jupyter&logoColor=white)
![SPSS](https://img.shields.io/badge/SPSS-7C3AED?style=flat-square&logo=ibm&logoColor=white)
![Salesforce](https://img.shields.io/badge/Salesforce-8B5CF6?style=flat-square&logo=salesforce&logoColor=white)

### AI / ML & Data Workflows

[![AI ML and Data Workflows](https://skillicons.dev/icons?i=py,sklearn,fastapi,postgres&theme=dark)](https://skillicons.dev)

![Machine Learning](https://img.shields.io/badge/Machine_Learning-312E81?style=flat-square)
![XGBoost](https://img.shields.io/badge/XGBoost-4338CA?style=flat-square)
![Predictive Analytics](https://img.shields.io/badge/Predictive_Analytics-4F46E5?style=flat-square)
![ETL](https://img.shields.io/badge/ETL_%26_Data_Quality-6D28D9?style=flat-square)
![Statistical Analysis](https://img.shields.io/badge/Statistical_Analysis-7C3AED?style=flat-square)

### Development & Automation

[![Development and Automation](https://skillicons.dev/icons?i=ts,react,nextjs,git,githubactions,vscode&theme=dark)](https://skillicons.dev)

![n8n](https://img.shields.io/badge/n8n-7C3AED?style=flat-square&logo=n8n&logoColor=white)
![Workflow Automation](https://img.shields.io/badge/Workflow_Automation-8B5CF6?style=flat-square)

</div>

---

## Data, BI & AI / ML Expertise

| Domain | Proficiency | Details |
|:---|:---:|:---|
| Business Intelligence | Advanced applied | KPI frameworks, executive dashboards, Power BI, Tableau, Looker, data storytelling |
| Data & Statistical Analysis | Advanced applied | Data cleaning, regression, clustering, factor analysis, reliability analysis, hypothesis testing |
| Applied Machine Learning | Advanced applied | XGBoost, logistic regression, decision trees, ensemble learning, feature engineering, temporal validation |
| Probabilistic Modeling | Applied | Elo systems, Dixon–Coles models, calibrated probabilities, Brier/RPS and log-loss evaluation |
| Simulation & Forecasting | Advanced applied | Vectorized Monte Carlo simulation, scenario analysis, demand and financial trend modeling |
| Data Quality & Automation | Advanced applied | ETL automation, schema validation, data provenance, anomaly detection, reproducible workflows |
| CRM & Business Analytics | Advanced applied | Segmentation, CLV, customer journeys, SERVQUAL/SERVPERF, supply-chain and commercial KPIs |
| Responsible AI Analysis | Applied | Evidence-based scoring, traceability, red flags, human review, and escalation logic |

---

## Featured Projects

<details open>
<summary><strong>CDD-Mundial — FIFA World Cup 2026 Forecasting System</strong></summary>

<br>

An end-to-end data science system that forecasts the 2026 FIFA World Cup with dynamic Elo ratings, Dixon–Coles modeling, calibrated XGBoost, and tournament-wide Monte Carlo simulation.

| Dimension | Details |
|:---|:---|
| **Stack** | Python · NumPy · pandas · XGBoost · Pandera · Pytest · Matplotlib |
| **Scale** | 48 teams · 104 fixtures · 49,000+ historical matches · thousands of tournament simulations |
| **Performance** | Temporal holdouts, log-loss, Brier/RPS, calibration gates, vectorized simulation |
| **Security** | Gitignored secrets, restricted-source policy, automated secret scanning, immutable raw captures |
| **Impact** | Reproducible live snapshots with full data/model lineage and promotion gates |
| **Repository** | [View source](https://github.com/jesusmiranda-analytics/Data-Science---World-Cup) |

The system uses strict, coercing Pandera contracts, canonical team identities, deterministic seeds, SHA-256 provenance manifests, and append-only production snapshots. Its ML upgrade is promoted only when it beats the structural baseline on out-of-sample calibration—not because a model is more complex.

</details>

<details>
<summary><strong>CNH Industrial — Supply Chain Risk & Inventory Analytics</strong></summary>

<br>

An applied Business Intelligence project developed with CNH Industrial to improve decision visibility for the Querétaro spare-parts warehouse across Case and New Holland operations.

| Dimension | Details |
|:---|:---|
| **Stack** | Oracle ERP data · Power BI · Data Consolidation · ABC Analysis · Forecasting |
| **Scale** | Global supply-chain context · lead times up to 180 days · multi-brand spare-parts inventory |
| **Performance** | Monitored 2.6–2.8 inventory turns against a target of 3 annual turns |
| **Risk** | Quantified 22% obsolescence exposure and the service-level vs. working-capital trade-off |
| **Impact** | Dynamic KPI dashboards and quantified recommendations for inventory decision-making |
| **Repository** | Private case · executive walkthrough available through [email](mailto:jesusm.12@yahoo.com) |

The analysis combined ERP data preparation, risk indicators, ABC classification, inventory rotation, service-level tracking, and forecasting alternatives to support decisions in a low-volume, high-variability environment.

</details>

<details>
<summary><strong>Medical Impact — Healthcare Service Quality Analytics</strong></summary>

<br>

An applied analytics and consulting project for a Mexican healthcare NGO, using 427 patient responses to identify the operational and human factors that drive perceived service quality.

| Dimension | Details |
|:---|:---|
| **Stack** | SPSS · Jamovi · Looker Studio · SERVPERF/SERVQUAL · Regression · Factor Analysis |
| **Scale** | 427 observations · 20 Likert items · rural and high-need communities |
| **Performance** | Durbin–Watson 1.83 · VIF 1.48–2.02 · statistically significant operational and trust dimensions |
| **Governance** | Ethical data treatment, retained contextual outliers, transparent assumption validation |
| **Impact** | Regional winner and national finalist with an executive dashboard and actionable service KPIs |
| **Repository** | Private academic case · analytical walkthrough available through [email](mailto:jesusm.12@yahoo.com) |

The reconstructed three-dimension model provided stronger actionable insight than the original framework, identifying operational conditions and clinical trust as significant predictors of overall satisfaction.

</details>

<details>
<summary><strong>EmotIA — AI-Driven EdTech MVP</strong></summary>

<br>

An AI-enabled SaaS concept for early detection of emotional instability in K–6 learners through evidence-based assessment, model-assisted inference, and role-based dashboards.

| Dimension | Details |
|:---|:---|
| **Stack** | Python · Machine Learning · Predictive Analytics · APIs · Role-based Dashboards |
| **Scale** | Campus winner → global finalist across 80+ international hackathons |
| **Performance** | End-to-end MVP spanning ingestion, inference, decision support, and presentation |
| **Security** | Role-oriented access concept, responsible escalation, human-centered decision support |
| **Impact** | Presented at IFE Conference 2026, an international education innovation forum |
| **Repository** | Private project · technical walkthrough available through [email](mailto:jesusm.12@yahoo.com) |

EmotIA was designed around a critical product principle: AI should surface weak signals early while keeping educators and specialists in control of consequential decisions.

</details>

---

## Experience

### Markets & Securities Services Operations Intern · HSBC México

**June 2026 — Present**

Applying data analysis, ETL automation, and AI-assisted decision logic to data-intensive banking operations.

- Designed and executed seven low-code ETL automation flows for transformation, cleansing, and validation, reducing analysts' manual processing time to approximately 5% of the previous effort.
- Contributed to an AI proof of concept for commercial-relationship assessment during onboarding, centered on evidence, scoring, traceability, and Financial Crime escalation.
- Defined LLM API requirements and designed an 18-node decision tree with scoring logic, red flags, and human-in-the-loop controls.
- Earned internal certification in HSBC's operational automation platform.

![ETL](https://img.shields.io/badge/ETL-312E81?style=flat-square)
![Automation](https://img.shields.io/badge/Automation-4338CA?style=flat-square)
![Data Quality](https://img.shields.io/badge/Data_Quality-4F46E5?style=flat-square)
![Responsible AI](https://img.shields.io/badge/Responsible_AI-6D28D9?style=flat-square)
![Banking Operations](https://img.shields.io/badge/Banking_Operations-7C3AED?style=flat-square)

### Project Manager · Intrasiap S.A. de C.V.

**May 2024 — November 2024**

Led a cross-functional certification initiative for a technology company seeking accreditation as a trusted industrial-strategy provider.

- Assessed operational capabilities and certification requirements.
- Coordinated internal stakeholders across technical, operational, and commercial workstreams.
- Developed communication and process-alignment strategies to strengthen delivery readiness.

![Project Management](https://img.shields.io/badge/Project_Management-312E81?style=flat-square)
![Stakeholders](https://img.shields.io/badge/Stakeholder_Alignment-4F46E5?style=flat-square)
![Process Design](https://img.shields.io/badge/Process_Design-6D28D9?style=flat-square)

---

## Achievements

| Recognition | Details |
|:---:|:---|
| **DigiEduHack 2025 — Regional Winner & Global Finalist** | EmotIA advanced from the Tecnológico de Monterrey campus competition to the global stage across 80+ international hackathons. |
| **Medical Impact Challenge — Regional Winner & National Finalist** | Built a statistically validated service-quality diagnostic from 427 responses for a healthcare NGO serving high-need communities. |
| **IFE Conference 2026 Presenter** | Presented the EmotIA AI EdTech MVP to an international education and innovation audience. |
| **Ginger Birra — Winning Brand Strategy** | Developed the selected Brand Wheel and evidence-backed positioning proposal for a Querétaro craft beverage brand. |
| **Social Leadership** | Coordinated 500+ volunteers and initiatives reaching 400+ children through Soñar Despierto Querétaro. |

---

## Certifications

### HSBC

![HSBC](https://img.shields.io/badge/HSBC-Operational_Automation_Platform_Certification-312E81?style=for-the-badge&logo=hsbc&logoColor=white)

### McKinsey.org

[![McKinsey Forward](https://img.shields.io/badge/McKinsey.org-Forward_Program-4338CA?style=for-the-badge&logoColor=white)](https://www.mckinsey.com/forward)

### LinkedIn Learning

[![Python OOP](https://img.shields.io/badge/LinkedIn_Learning-Python_OOP-4F46E5?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/learning/)
[![Logistic Regression](https://img.shields.io/badge/LinkedIn_Learning-Logistic_Regression-6366F1?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/learning/)
[![Decision Trees](https://img.shields.io/badge/LinkedIn_Learning-Decision_Trees-7C3AED?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/learning/)
[![Ensemble Learning](https://img.shields.io/badge/LinkedIn_Learning-Ensemble_Learning-8B5CF6?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/learning/)

### Google · DataCamp · Salesforce · IBM

[![Google Looker](https://img.shields.io/badge/Google-Introduction_to_Looker-312E81?style=flat-square&logo=looker&logoColor=white)](https://www.cloudskillsboost.google/)
[![DataCamp](https://img.shields.io/badge/DataCamp-SQL_%7C_Python_%7C_R_%7C_BI-4338CA?style=flat-square&logo=datacamp&logoColor=white)](https://www.datacamp.com/)
[![Salesforce](https://img.shields.io/badge/Salesforce-CRM_%2B_AI_%2B_Data-6D28D9?style=flat-square&logo=salesforce&logoColor=white)](https://trailhead.salesforce.com/)
[![IBM Design Thinking](https://img.shields.io/badge/IBM-Design_Thinking-7C3AED?style=flat-square&logo=ibm&logoColor=white)](https://www.ibm.com/design/thinking/)

---

## Coding Profiles

<div align="center">

[![GitHub](https://img.shields.io/badge/GitHub-Data_%26_BI_Portfolio-312E81?style=for-the-badge&logo=github&logoColor=white)](https://github.com/jesusmiranda-analytics)

![LeetCode](https://img.shields.io/badge/LeetCode-Profile_Not_Public-4338CA?style=for-the-badge&logo=leetcode&logoColor=white)
![GeeksforGeeks](https://img.shields.io/badge/GeeksforGeeks-Profile_Not_Public-4F46E5?style=for-the-badge&logo=geeksforgeeks&logoColor=white)
![HackerRank](https://img.shields.io/badge/HackerRank-Profile_Not_Public-6D28D9?style=for-the-badge&logo=hackerrank&logoColor=white)
![CodeChef](https://img.shields.io/badge/CodeChef-Profile_Not_Public-7C3AED?style=for-the-badge&logo=codechef&logoColor=white)

</div>

---

## GitHub Analytics

<div align="center">

[![GitHub Profile Details](https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=jesusmiranda-analytics&theme=tokyonight)](https://github.com/vn7n24fzkq/github-profile-summary-cards)

[![GitHub Stats](https://github-profile-summary-cards.vercel.app/api/cards/stats?username=jesusmiranda-analytics&theme=tokyonight)](https://github.com/vn7n24fzkq/github-profile-summary-cards)
[![Repositories by Language](https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=jesusmiranda-analytics&theme=tokyonight)](https://github.com/vn7n24fzkq/github-profile-summary-cards)

</div>

---

## GitHub Highlights

<div align="center">

[![Productive Time](https://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=jesusmiranda-analytics&theme=tokyonight&utcOffset=-6)](https://github.com/vn7n24fzkq/github-profile-summary-cards)
[![Most Committed Language](https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=jesusmiranda-analytics&theme=tokyonight)](https://github.com/vn7n24fzkq/github-profile-summary-cards)

</div>

---

## Contribution Activity

<div align="center">

[![Contribution Graph](https://github-readme-activity-graph.vercel.app/graph?username=jesusmiranda-analytics&bg_color=0D1117&color=A78BFA&line=7C3AED&point=FFFFFF&area=true&hide_border=true&custom_title=Contribution%20Activity)](https://github.com/ashutosh00710/github-readme-activity-graph)

</div>

---

## Contribution Snake

<div align="center">

![Contribution Snake](https://raw.githubusercontent.com/jesusmiranda-analytics/a01769572-art/gh-pages/github-contribution-grid-snake-dark.svg)

</div>

---

## Current Focus

```yaml
learning:
  - advanced analytics and machine learning evaluation
  - banking, markets, and financial systems
  - executive data storytelling and BI strategy

building:
  - decision-oriented dashboards and KPI frameworks
  - reproducible forecasting and predictive models
  - automated ETL and data-quality workflows

exploring:
  - applied AI for business decision support
  - human-in-the-loop AI operations
  - quantitative finance and market intelligence

open_to:
  - Data Analyst opportunities
  - Business Intelligence Analyst opportunities
  - Junior Data Scientist and AI/ML Analyst roles
  - analytics projects with measurable business impact
```

---

## Connect

<div align="center">

[![Gmail](https://img.shields.io/badge/Gmail-jesusm.12%40yahoo.com-7C3AED?style=for-the-badge&logo=gmail&logoColor=white)](mailto:jesusm.12@yahoo.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Jesús_Miranda-4F46E5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/jes%C3%BAs-alfonso-miranda-ortega-98a381321/)
[![GitHub](https://img.shields.io/badge/GitHub-jesusmiranda--analytics-312E81?style=for-the-badge&logo=github&logoColor=white)](https://github.com/jesusmiranda-analytics)
[![Portfolio](https://img.shields.io/badge/Portfolio-Live_Product-6D28D9?style=for-the-badge&logo=vercel&logoColor=white)](https://mi-portafolio-xi-ten.vercel.app)

</div>

---

<div align="center">

*"Data becomes valuable when it improves a decision."*

![Footer](https://capsule-render.vercel.app/api?type=waving&color=0:312e81,45:6d28d9,100:a855f7&height=120&section=footer)

</div>
